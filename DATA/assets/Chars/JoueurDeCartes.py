    ...
    def animation_attack(self,attack,inputs,stage,other):
        ...
        if attack == "UpB":
            ...
            if self.frame == 6: # Hitbox frame 6-15
                self.vx = 0
                self.vy = -24
                pygame.mixer.Sound(f"DATA/Musics/SFX/boings/boing ({randint(2,8)})").play()
        ...
        if attack == "Jab":
            if self.frame == 2 : # Frame 2-3
                self.active_hitboxes.append(Hitbox(32,20,44,48,3*pi/4,0.7,0.3,1/1000,5,2,self,False,boum=-2,sound="hits/hit"))
        ...
        if attack == "UpTilt":
            ...
            if self.frame == 8 :
                self.active_hitboxes.append(Hitbox(24,0,32,32,pi/2,9,8.5,1/500,13,11,self,False,sound="wooshs/mini woosh"))
        ...
        if attack == "ForwardAir":
            if self.frame == 9 : # Frame 9-10
                self.active_hitboxes.append(Hitbox(24,28,64,12,pi/6,12,11.3,1/150,16,2,self,False,sound="other/electric cable sound"))
        ...
        if attack == "DownAir":
            ...
            if self.frame == 42 :
                self.active_hitboxes.append(Hitbox(-8,128,64,64,-pi/2,5,6.5,1/200,20,2,self,False,sound="hits/cool hit"))
        ...
        if attack == "UpSmash":
            ...
            if self.frame == 11 :
                ...
                self.charge = min(self.charge,100)
                self.active_hitboxes.append(Hitbox(0,-50,48,48,pi/3,9+7*(self.charge/150),14,1/80,13+7*(self.charge/100),13,self,False,sound="hits/cool hit"))
        ...
        if attack == "DownSmash":
            ...
            if self.frame > 3 and self.frame < 6  and smash and self.charge < 200 : # Chargement jusqu'à 200 frames
                ...
                self.charge = min(self.charge,100)
                self.active_hitboxes.append(Hitbox(-10,100,100,32,5*pi/13,12+10*(self.charge/200),10.5,1/250,12+8*(self.charge/300+1),8,self,False,sound="hits/punch2"))
        ...
        if attack == "DashAttack":
            if self.frame > 5 and self.frame%10 == 0 and self.frame < 55: # Active on 10-11/20-21/30-31/40-41/50-51/
                ...
                if self.grounded :
                    ...
                    self.vx -= self.dashspeed*signe(self.direction)*5
                self.active_hitboxes.append(Hitbox(10,32,64,64,pi/5,8,3.5,1/350,7,3,self,False,sound="hits/mini hit"))
...
class Carte():
    def __init__(self,x,y,angle,number,own:Air_President) -> None:
        if number == "R":
            pygame.mixer.Sound("DATA/Musics/SFX/other/Its-pronounced-rules").play()
            self.sound = pygame.mixer.Sound("DATA/Musics/SFX/hits/cool hit")
            ...
            self.sound = pygame.mixer.Sound("DATA/Musics/SFX/wooshs/woosh")
...
class Blahaj():
    def __init__(self,color,own:Air_President,stage):
        # Blahaj
        pygame.mixer.Sound("DATA/Musics/SFX/wooshs/encore un woosh").play()
        self.sound = pygame.mixer.Sound("DATA/Musics/SFX/boings/boing")
